...
def single_scale_retinex_process(image,sigma=30):
...
def capture():
    ...
    if capture_image is not None:
        ...
        retinex_img = single_scale_retinex_process(processed_image)
        yolo_obj_draw(retinex_img, threshold=0.2)
        yolo_img_rgb = cv2.cvtColor(retinex_img, cv2.COLOR_BGR2RGB)
